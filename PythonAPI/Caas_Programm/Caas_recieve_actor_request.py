...
def on_transport_request(vehicle1_id, vehicle2_id, destination):
        ...
        goal_waypoint = spawn_points[int(destination)]

        #Get Vehicle 1 and 2
        vehicle1 = world.get_actor(int(vehicle1_id))
        vehicle2 = world.get_actor(int(vehicle2_id))
        ...
        vehicle2_waypoint = vehicle2.next(15.0)

        while vehicle2_waypoint != vehicle1_waypoint:
                world.tick()
                ts = world.wait_for_tick()
                vehicle1_waypoint = vehicle1.next(0)

                print("Vehicle 1 Waypoint: " + vehicle1_waypoint)
                print("Vehicle 2 Waypoint: " + vehicle1_waypoint)
...
def on_connect(client, userdata, flags, rc):
  ...
  client.subscribe("topic/transport/request")
...
def on_message(client, userdata, msg):
  ...
  vehicle1_id = y["vehicle1_id"]
  vehicle2_id = y["vehicle2_id"]
  destination = y["destination"]
  on_transport_request(vehicle1_id, vehicle2_id, destination)
